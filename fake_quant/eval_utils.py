...
def evaluator(model, testenc, dev, args):
    ...
    if 'opt' in args.model:
        opt_type = True
        llama_type = False
    elif 'meta' in args.model or "gemma" in args.model or "Qwen" in args.model or "mistral" in args.model:
        llama_type = True
        opt_type = False
    else:
    ...
    nsamples = input_ids.numel() // args.val_seqlen  # The tail is truncated.
    input_ids = input_ids[:, :nsamples * args.val_seqlen].view(nsamples, args.val_seqlen).to(dev)  # (nsamples, seqlen)
    ...
    inps = torch.zeros(
        (nbatches, batch_size, args.val_seqlen, model.config.hidden_size), dtype=dtype, device=dev
    )
    inps = [0] * nbatches
