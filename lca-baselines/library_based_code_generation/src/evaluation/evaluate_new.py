...
def evaluate(model: ExampleGenerationModel, metrics: list[Metric], data_path: str, args=None):
    print(f"Evaluating model {model.name()}")

    dataset = load_dataset("JetBrains-Research/lca-library-based-code-generation", split="test")
    n_samples = len(dataset)

    if args is not None and args.save_name != "":
        evaluation_result_path = os.path.join(data_path, model.name(), args.save_name)
    else:
    ...
    for i, sample in tqdm(enumerate(dataset)):
        ...
        if os.path.exists(generated_file) and args.dont_overwrite:
...
def evaluate_hf(model_name, use_bm25=False, args=None):
    evaluate(HFChatModel(model_name, use_bm25, args), [ChrF()], "results", args=args)
...
if __name__ == "__main__":
    ...
    evaluate_hf(args.model, use_bm25=False, args=args)
