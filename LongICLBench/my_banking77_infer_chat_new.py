...
parser.add_argument("-m", "--model", type=str, help="model name to test", default="meta-llama/Meta-Llama-3-8B-Instruct")
...
parser.add_argument("--save_name", type=str, default="", help="name of the output file")
...
assert "llama" in args.model or "mistral" in args.model, "model name should contain llama or mistral"
...
for example in eva_data[:args.test_number]:
    ...
    processed_chat = demo_prompt.split("service query:")
    ...
    for chat in processed_chat:
        chat = chat.split("intent category:")
        messages.append(
            {"role": "user", "content": "service query: " + chat[0].strip()}
        )
        messages.append(
            {"role": "assistant", "content": "intent category: " + chat[1].strip()}
        )
        
    messages.append(
        {"role": "user", "content": "service query: " + example['text']}
    )
